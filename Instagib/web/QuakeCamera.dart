...
class QuakeCamera extends Animatable {
  ...
  void animate(double elapsed) {
    ...
    if (movementX != 0) camera.transform.rotate(movementX * 0.006, qup);
  ...
  void groundCheck() {
    ...
    if (velocity[2] > 0 && velocity.dot(groundTrace.plane.normal) > 10) {
      // jumping
      onGround = false;
      return;
    }
    ...
    for (bumpcount = 0; bumpcount < numbumps; ++bumpcount) {
      ...
      for (int i = 0; i < planes.length; ++i) {
        ...
        for (int j = 0; j < planes.length; j++) {
          ...
          if (clipVelocity_.dot(planes[j]) >= 0.1) {
            continue;
          } // move doesn't interact with the plane
          ...
          if (clipVelocity_.dot(planes[i]) >= 0) {
            continue;
          }
          ...
          for (int k = 0; k < planes.length; ++k) {
            ...
            if (clipVelocity_.dot(planes[k]) >= 0.1) {
              continue;
            } // move doesn't interact with the plane
  ...
  void stepSlideMove(bool gravity) {
    ...
    if (velocity[2] > 0 && (ssmTrace.fraction == 1.0 || ssmTrace.plane.normal.dot(up) < 0.7)) {
      return;
    }
